...
public class DNA {
    ...
    public void createDatabaseOfProfiles ( String filename ) {
        ...
        StdIn.readLine(); // skip the empty newline
    ...
    public Profile createUnknownProfile ( String filename ) {
        ...
        Profile unknownProfile = new Profile(name, S1_STRs, S2_STRs, sequence1, sequence2);
        return unknownProfile; // update the return value
    }
    ...
    public STR findSTRInSequence ( String sequence, String STR ) {
        ...
        while (i < sequence.length()) {
            ...
            if (i + STR.length() <= sequence.length() && sequence.substring(i, i + STR.length()).equals(STR)) {
                currRepeat++;
                i += STR.length();
    ...
    public ArrayList<Profile> findMatchingProfiles ( STR[] unknownProfileS1_STRs ) {
        ...
        for (Profile profile : database) {
            if (identicalSTRs(unknownProfileS1_STRs, profile.getS1_STRs())) {
         ...
         for ( int p1 = 0; p1 < possibleParent1.size(); p1++ ) {
            ...
            for ( int p2 = 0; p2 < possibleParent2.size(); p2++ ) {
                ...
                if ( !possibleParent1.get(p1).equals(possibleParent2.get(p2)) ) {
                    ...
                    } else if (punnetSquare(possibleParent1.get(p1).getS2_STRs(), S1_STRs, possibleParent2.get(p2).getS1_STRs(), S2_STRs)) {
                        parentList.add(possibleParent1.get(p1));
                        parentList.add(possibleParent2.get(p2));
                    } else if (punnetSquare(possibleParent1.get(p1).getS1_STRs(), S1_STRs, possibleParent2.get(p2).getS2_STRs(), S2_STRs)) {
                        parentList.add(possibleParent1.get(p1));
                        parentList.add(possibleParent2.get(p2));
                    } else if (punnetSquare(possibleParent1.get(p1).getS2_STRs(), S1_STRs, possibleParent2.get(p2).getS2_STRs(), S2_STRs)) {
                        parentList.add(possibleParent1.get(p1));
                        parentList.add(possibleParent2.get(p2));
                    }
                }
